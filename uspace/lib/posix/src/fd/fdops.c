...
typedef struct __posix_fd _fd_t;
...
static int _grow_fds(int min_len)
{
	/* We try to double the number of entries unless we've hit the limit. */
	int old_len = _fds.len;
	int new_len;
	...
	_fd_t **new_fds = realloc(_fds.entries, new_len * sizeof(*_fds.entries));

	/* Try to allocate at least min_len if we can't get the preferred size. */
	if (new_fds == NULL) {
		new_len = min_len;
		new_fds = realloc(fds, new_len * sizeof(*_fds.entries));
	}
	...
	_fds.entries = new_fds;
	_fds.len = new_len;
	return 0;
}

static int _alloc_fd_locked_2(_fd_t *f, int fd)
{
	assert(fd >= 0);
	assert(fd < _fds.len);
	assert(_fds.entries[fd] == NULL);

	if (fd == _fds.lowest_free) {
		_fds.lowest_free += 1;
	}
...
static int _alloc_fd_locked(_fd_t *f, int min_fd)
{
	int len = _fds.len;
	_fd_t **fds = _fds.entries;
	...
	if (min_fd <= _fds.lowest_free) {
		min_fd = _fds.lowest_free;
		is_lowest = true;
	}
	...
	if (is_lowest) {
		_fds.lowest_free = min_fd + 1;
	}

	return _alloc_fd_locked_2(f, min_fd);
}

int __alloc_fd(_fd_t *f)
{
	fibril_mutex_lock(&_fds.mutex);
	int retval = _alloc_fd_locked(f, 0);
	fibril_mutex_unlock(&_fds.mutex);
	return retval;
}

static _fd_t *_get_file_locked(int fd)
{
	if (fd < 0 || fd >= _fds.len) {
		errno = EBADF;
		return NULL;
...
static _fd_t *_take_file_locked(int fd)
{
	if (fd < 0 || fd >= _fds.len) {
		errno = EBADF;
		return NULL;
...
static _fd_t *_take_file(int fd)
{
	fibril_mutex_lock(&_fds.mutex);
	_fd_t *f = _take_file_locked(fd);
	fibril_mutex_unlock(&_fds.mutex);
	return f;
}

static int _put_file(_fd_t *f)
{
	if (f == NULL) {
		return -1;
	}
...
static int _with_file(int fd, void (*op)(_fd_t *, void *), void *data)
{
	fibril_mutex_lock(&_fds.mutex);
	_fd_t *f = _get_file_locked(fd);
	fibril_mutex_unlock(&_fds.mutex);
