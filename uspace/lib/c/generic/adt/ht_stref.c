...
bool ht_stref_create(hash_table_t *ht)
{
...
void ht_stref_destroy(hash_table_t *ht, void (*destroy_fn)(void *))
{
	if (destroy_fn)
...
bool ht_stref_insert(hash_table_t *ht, const char *key, void *value)
{
...
void *ht_stref_set(hash_table_t *ht, const char *key, void *value)
{
	assert(ht != NULL);
	assert(key != NULL);
...
void *ht_stref_get(hash_table_t *ht, const char *key)
{
...
void *ht_stref_remove(hash_table_t *ht, const char *key)
{
	ht_link_t *link = hash_table_find(ht, key);
	if (!link)
		return NULL;
